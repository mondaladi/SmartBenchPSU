//This version uses PWM.h for precise high-frequency PWM control.
...
#include <Bounce2.h>
#include <Encoder.h>
#include <PWM.h> // Library for precise PWM frequency control
...
int32_t frequency = 150000; // PWM frequency set to 150 kHz
...
void setup() {
  // Initialize PWM with a high frequency for stable output voltage control
  InitTimersSafe();
  bool success = SetPinFrequencySafe(controlPin, frequency);
  
  // If PWM frequency setup is successful, configure the control pin
  if (success) {
  ...
  Serial.println("INA219 successfully initialized");
...
void constantCurrent() {
  ...
  // Apply PWM signal using pwmWrite from PWM.h library
  pwmWrite(controlPin, pwm);
}
